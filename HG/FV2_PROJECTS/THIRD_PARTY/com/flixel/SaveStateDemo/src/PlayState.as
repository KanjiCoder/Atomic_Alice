package 
{
	...
	public class PlayState extends FlxState
	{
		...
		override public function create():void
		{
			...
			gameSave.bind("SaveDemo_cookies"); //<<doesn't look like name has to match class name "SaveDemo".
			...
			for (var i:int = 0; i < numBoxes; i++) {
				...
				if (gameSave.data.boxPositions != null){
					box = new FlxButton(gameSave.data.boxPositions[i].x, gameSave.data.boxPositions[i].y, (i + 1).toString());
				...
				//If not, oh well we'll just put them in the default locations
					...
					box = new FlxButton((i * 35) + 9, 50, (i+1).toString());
					if (i * 35 > 360){
						box.y = 85;
						box.x = (i * 35 -2) - 339;
					}
		...
		private function onSave():void {
			...
			if (gameSave.data.boxPositions == null) {
				...
				for each(a in boxGroup.members) {
